...
if (!class_exists('Webtoffee_Product_Feed_Sync_TiktokShop_Export')) {

    class Webtoffee_Product_Feed_Sync_TiktokShop_Export extends Webtoffee_Product_Feed_Product {
        ...
        public $tiktok_xls_keys = array();

        public function __construct($parent_object) {

            $this->parent_module = $parent_object;
            $dimension_unit = get_option('woocommerce_dimension_unit');
            $this->tiktok_xls_keys = array(
                'title' => 'product name',
                'brand' => 'Brand',
                ...
                'identification_type' => 'Identifier Code Type',
                'identifier_code' => 'identifier code',
                'package_weight' => 'Package Weight ('.get_option('woocommerce_weight_unit').')',
                'package_length' => 'Package Length ('.$dimension_unit.')',
                'package_width' => 'Package Width ('.$dimension_unit.')',
                'package_height' => 'Package Height ('.$dimension_unit.')',
            );            
            
            $max_variations = $this->parent_module->get_max_variations_count_for_all_variable_products();
            if( $max_variations > 0 ){                            
                for($i=1; $i<=$max_variations; $i++){
                    $this->tiktok_xls_keys['variation_'.$i] = 'Variation '.$i;                                
                }
                $this->tiktok_xls_keys['variant_image'] = 'Variant Image';
            }            
        ...
        public function prepare_header() {
            ...
            foreach ($export_columns as $key => $value) {
                if(isset($this->tiktok_xls_keys[$key])){
                    $export_columns[$key] = $this->tiktok_xls_keys[$key];                    
                }
            }
        ...
        protected function generate_row_data_wc_lower($product_object) {
            ...
            foreach ($export_columns as $key => $value) {
                if (method_exists($this, $value)) {
                    ...
                    if(isset($this->tiktok_xls_keys[$value])){
                        $row[$this->tiktok_xls_keys[$value]] = $column_data;
                    }else{
                        $row[$key] = $column_data;
                    }
        ...
        public function get_shipping() {
            ...
            $shipping_obj = new Webtoffee_Product_Feed_Basic_Shipping($this->product, 'tiktok', $this->form_data);
                ...
                foreach ($category_path as $category) {
                    ...
                    if ($tiktok_category_id) {
                        ...
                        if (false === $tiktok_category_list) {
                            $tiktok_category_list = Webtoffee_Product_Feed_Sync_TiktokShop::get_category_array();
        ...
        public function variation_1($catalog_attr, $product_attr, $export_columns) {
            return $this->wt_variation_option($catalog_attr, $product_attr, $export_columns, 1);
        }
        public function variation_2($catalog_attr, $product_attr, $export_columns) {
            return $this->wt_variation_option($catalog_attr, $product_attr, $export_columns, 2);
        }
        public function variation_3($catalog_attr, $product_attr, $export_columns) {
            return $this->wt_variation_option($catalog_attr, $product_attr, $export_columns, 3);
        }
        public function variation_4($catalog_attr, $product_attr, $export_columns) {
            return $this->wt_variation_option($catalog_attr, $product_attr, $export_columns, 4);
        }
        public function variation_5($catalog_attr, $product_attr, $export_columns) {
            return $this->wt_variation_option($catalog_attr, $product_attr, $export_columns, 5);
        }
        public function variation_6($catalog_attr, $product_attr, $export_columns) {
