...
if (!class_exists('Webtoffee_Product_Feed_Sync_Heureka_Export')) {

    class Webtoffee_Product_Feed_Sync_Heureka_Export extends Webtoffee_Product_Feed_Product {

        public $parent_module = null;
        public $product;
        public $current_product_id;
        public $form_data;
        public $heureka_xml_keys = array();

        public function __construct($parent_object) {

            $this->parent_module = $parent_object;
            
            $this->heureka_xml_keys = array(
                'id' => 'ITEM_ID',
                'title' => 'PRODUCTNAME',
                'name' => 'PRODUCT',
        ...
        public function prepare_header() {
            ...
            foreach ($export_columns as $key => $value) {
                if(isset($this->heureka_xml_keys[$key])){
                    $export_columns[$key] = $this->heureka_xml_keys[$key];                    
                }
            }
        ...
        public function prepare_data_to_export($form_data, $batch_offset, $step) {
            ...
            if ($batch_offset < $export_limit) {
                ...
                // If include tags is selected and variable products are under those tags, the variations will not be returned by the WC query.
                if( 'include_tag' === $tag_filter_type && !empty($inc_exc_tag) ){
                    $temp_tagged_prod_ids = $products_ids;
                    foreach ($temp_tagged_prod_ids as $key => $product_id) {
                        $product = wc_get_product($product_id);
                        if ($product->is_type('variable')) {
                            $variations = $product->get_available_variations();
                            $variations_ids = wp_list_pluck($variations, 'variation_id');
                            foreach ($variations_ids as $variations_id) {
                                if(in_array($variations_id, $temp_tagged_prod_ids) && 'product_variation' === get_post_type($variations_id) ){
                                    continue;
                                }
                                $products_ids[] = $variations_id;
                            }
                ...
                foreach ($products_ids as $key => $product_id) {
                    ...
                    // Skip variations that belongs to a specific tags that is excluded in filter
                    if ( $product->is_type('variation') and !empty($inc_exc_tag) and 'exclude_tag' === $tag_filter_type ) {
                        $parent_id = $product->get_parent_id();
                        if (has_term($inc_exc_tag, 'product_tag', $parent_id)) {
                            continue;
                        }
                    }                    
                    ...
                    $product_array_processed = $this->generate_row_data_wc_lower($product);
                    
                    if( $product->is_type( 'variation' ) ){                        
                        $product_data = $this->variant_attribute_option();
                        if(!empty( $product_data )){
                            $product_array_processed['PARAM'] = array();
                            $param_count = 0;
                            foreach($product_data as $product_data_single){
                                $product_array_processed['PARAM'][$param_count]['PRAM_NAME'] = $product_data_single['label'];
                                $product_array_processed['PARAM'][$param_count]['VAL'] = $product_data_single['value'];
                                $param_count++;
                            }
                            
                        }
                    }
                    $product_array[] = $product_array_processed;
                }
            }
        ...
        protected function generate_row_data_wc_lower($product_object) {
            ...
            foreach ($export_columns as $key => $value) {
                if (method_exists($this, $value)) {
                    ...
                    if(isset($this->heureka_xml_keys[$value])){
                        $row[$this->heureka_xml_keys[$value]] = $column_data;
                    }else{
                        $row[$key] = $column_data;
                    }
        ...
        public function name($catalog_attr, $product_attr, $export_columns) {
            ...
            return apply_filters("wt_feed_heureka_product_title", $title, $this->product);
        }        
        ...
        public function link($catalog_attr, $product_attr, $export_columns) {                        
            ...
            return apply_filters("wt_feed_heureka_product_link", $link, $this->product);
        }        
        ...
        public function image_link_alternate($catalog_attr, $product_attr, $export_columns){
            ...
            return apply_filters('wt_feed_heureka_product_alternate_image', $image_alternate, $this->product);
        }
        /**
         * EAN
         * 
	...
	public function ean($catalog_attr, $product_attr, $export_columns){
            ...
            return apply_filters('wt_feed_heureka_product_ean', $ean, $this->product);
	}        
        ...
        public function currency($catalog_attr, $product_attr, $export_columns) {
            ...
            if ( ( class_exists('WCML_Multi_Currency') || class_exists('WOOCS') || class_exists('WOOMULTI_CURRENCY_F') ) && !empty( $this->form_data['post_type_form_data']['wt_pf_export_post_currency'] ) ) {
       ...
       public function colour($catalog_attr, $product_attr, $export_columns) {
        ...
        public function variant_attribute_option() {
            ...
            return apply_filters("wt_feed_heureka_variant_attributes", $product_data, $this->product);
        }        
