...
if(!class_exists('Webtoffee_Product_Feed_Sync_Google_Export')){
class Webtoffee_Product_Feed_Sync_Google_Export extends Webtoffee_Product_Feed_Product {
	...
	public function gender($catalog_attr, $product_attr, $export_columns) {
            ...
            if ( !$gender ) {
                $gender = get_post_meta($this->product->get_id(), '_wt_google_gender', true);
            }
            if ( !$gender && $this->product->is_type('variation') ) {
                ...
                if (!$attributes) {
                    return apply_filters('wt_feed_google_product_gender', $gender, $this->product);
                }
        ...
        public function size($catalog_attr, $product_attr, $export_columns) {
            ...
            if ( !$size ) {
                $size = get_post_meta($this->product->get_id(), '_wt_google_size', true);
            }
            if ( !$size && $this->product->is_type('variation')) {
                ...
                if (!$attributes) {
                    return apply_filters('wt_feed_google_product_size', $size, $this->product);
                }
        ...
        public function color($catalog_attr, $product_attr, $export_columns) {
            ...
            if ( !$color ) {
                $color = get_post_meta($this->product->get_id(), '_wt_google_color', true);
            }
            if ( !$color && $this->product->is_type('variation') ) {
                ...
                if (!$attributes) {
                    return apply_filters('wt_feed_google_product_color', $color, $this->product);
                }
