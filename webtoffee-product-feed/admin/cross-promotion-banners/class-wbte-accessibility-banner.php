...
if ( ! class_exists( 'Wbte_Accessibility_Banner' ) ) {
	...
	class Wbte_Accessibility_Banner {
		 ...
		 * @var string
		 */
		private static $plugin_prefix = 'wt_product_feed';
		...
		public static function install_plugin() {
			try {
				...
				$response = array(
					'status'  => false,
					'message' => esc_html__( 'Something went wrong. Please try again.', 'webtoffee-product-feed' ),
				);

				if ( ! current_user_can( 'install_plugins' ) ) {
					$response['message'] = esc_html__( 'You do not have sufficient permissions to install plugins.', 'webtoffee-product-feed' );
				...
				if ( ! isset( $api->download_link ) ) {
					$response['message'] = esc_html__( 'Plugin download link not found.', 'webtoffee-product-feed' );
					wp_send_json( $response );
					return;
				}
		...
		public static function activate_plugin() {
			...
			if ( ! current_user_can( 'activate_plugins' ) ) {
				$result['msg'] = esc_html__( 'You do not have sufficient permissions to activate plugins.', 'webtoffee-product-feed' );
				return $result;
			}
			...
			if ( file_exists( WP_PLUGIN_DIR . '/' . self::$plugin_file ) ) {
				...
				if ( is_wp_error( $activation_result ) ) {
					$result['status'] = false;
					$result['msg']    = esc_html__( 'Activation failed.', 'webtoffee-product-feed' );
				} else {
					$result['status'] = true;
				}
			}
		...
		public function show_banner_notice() {
			...
			<div id="wbte_accessibility_banner" class="notice">
				<div class="wbte_accessibility_banner_main">
					<img src="<?php echo esc_url( plugin_dir_url( __FILE__ ) . 'assets/images/accessyes.svg' ); ?>" alt="<?php esc_attr_e( 'Accessibility Widget', 'webtoffee-product-feed' ); ?>" style="width: 48px; height: 48px;">
					<div class="wbte_accessibility_banner_content">
						<p class="wbte_accessibility_banner_content_title">
							...
							printf(
								...
								esc_html__( 'Make your site inclusive with %1$s AccessYes accessibility widget. %2$s', 'webtoffee-product-feed' ),
						...
						<p style="font-size: 14px;">
							<?php
							esc_html_e( 'A lightweight accessibility widget to improve accessibility and usability for every users. Set it up in minutes for free.', 'webtoffee-product-feed' );
							?>
						</p>
					</div>
				</div>
				<div class="wbte_accessibility_banner_actions">
					<button class="button button-primary" id="wbte_accessibility_banner_install_btn">
						...
						<span class="wbte_accessibility_banner_install_btn_text">
							<?php esc_html_e( 'Install & activate', 'webtoffee-product-feed' ); ?>
						</span>
					</button>
					<a href="#" class="wbte_accessibility_banner_maybe_ltr">
						<?php esc_html_e( 'Remind me later', 'webtoffee-product-feed' ); ?>
					</a>
				</div>
				<span class="wbte_accessibility_banner_dismiss_btn">
					<?php esc_html_e( 'Dismiss', 'webtoffee-product-feed' ); ?>
		...
		public static function enqueue_admin_scripts() {
			...
			$error_msg = esc_js( __( 'Something went wrong. Please try again.', 'webtoffee-product-feed' ) );
			?>
			<style>
				...
				$( document ).ready( function() {
					...
					function setButtonLoading( isLoading ) {
						if ( isLoading ) {
							...
							btnTextSpan.text( '<?php echo esc_js( __( 'Installing...', 'webtoffee-product-feed' ) ); ?>' );
					...
					banner.on( 'click', '.wbte_accessibility_banner_dismiss_btn', function(e) {
						...
						$.ajax( {
							...
							error: function() {
								removeBannerLoader();
								wt_fbfeed_notify_msg.error( '<?php echo esc_js( $error_msg ); ?>' );
							}
						} );
					} );
						...
						$.ajax( {
							...
							success: function( response ) {
								if ( response.status ) {
									...
									wt_fbfeed_notify_msg.success( '<?php echo esc_js( __( 'Plugin installed successfully.', 'webtoffee-product-feed' ) ); ?>' );
								} else {
									setButtonLoading( false );
									const msg = ( response.message && response.message.length ) ? response.message : '<?php echo esc_js( $error_msg ); ?>';
									wt_fbfeed_notify_msg.error( msg );
								}
							},
							error: function() {
								setButtonLoading( false );
